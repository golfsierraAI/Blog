...
// ❌ Bad: Token in GET parameters
<a href="/action?csrf_token=<?php echo $token; ?>">Click here</a>

// ✅ Good: Token in POST body or headers
<form method="POST">
    <input type="hidden" name="csrf_token" value="<?php echo $token; ?>">
</form>

// ✅ Good: Token in custom header
fetch('/api/action', {
    method: 'POST',
    headers: {
        'X-CSRF-Token': token,
...
// ✅ Good: Using crypto.randomUUID() (Node.js 14.17.0+)
...
```javascript
// ❌ Bad: Only checking presence
if (req.body.csrf_token) {
    // Process request - vulnerable to empty string attacks
}

// ❌ Bad: Simple string comparison (timing attacks)
if (req.body.csrf_token === req.session.csrf_token) {
    // Vulnerable to timing attacks
}

// ✅ Good: Proper validation with timing-safe comparison
const crypto = require('crypto');
...
```javascript
// ❌ Bad: Not considering subdomains
app.use((req, res, next) => {
    const origin = req.get('Origin');
...
// ✅ Good: Proper subdomain handling
